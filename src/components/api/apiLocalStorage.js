...
export const apiDialogs = { // объект с методами api для DialogList
  _setMessages2LS: (Dialog_2, myId, userId) => { // метод отправки измененного диалога в LocalStorage и считывания его же
    const LocalStoragedialogUpdateTime1 =
      GetDate().Day + "." +
      GetDate().Month + "" +
      GetDate().Year + " " +
      GetDate().Hour + ":" +
    ...
    const dialogNameLocal = myId > userId ? "Dialog_" + myId + "_" + userId : "Dialog_" + userId + "_" + myId;  // задать имя диалога для запроса
    const dialogUpdateTimeLocal = myId > userId ? "Dialog_" + myId + "_" + userId + "_UpdateTime" : "Dialog_" + userId + "_" + myId + "_UpdateTime";// задать имя времени обновления диалога для запроса
    ...
    const Dialog_3 = apiDialogs.getDialog(myId, userId); // считать записанный массив с LocalStorage
  ...
  deleteMessage: (messageID, myId, userId) => {
    const Dialog_1 = apiDialogs.getDialog(myId, userId); // получить данные Dialog_25528_1079 с LocalStorage
    const Dialog_2 = Dialog_1.filter(message => message.id !== messageID) // отфильтровали массив, удалив сообщение с нужным ШВ

    const Dialog_3 = apiDialogs._setMessages2LS(Dialog_2, myId, userId)// отправить измененый массив на сервер (LocalStorage)
    ...
    if (Dialog_3.length===0) { // если после удаления сообщений их в диалоге не осталось
      ...
      const dialogListUserId1 = "DialogList_" + userId // задать имя DialogList
      ...
      const Data2 = Data1.filter(d=>d.userId!==myId) // оставить в диалогЛисте пользователя все диалоги, ктоме того,в котором 0 сообщений, кроме
  ...
  updateDialogListUserId: (userId1, userId2, Name2, Photo2) => { // запись в сервер данные о том, что у конкретного пользователя есть диалоги
    const dialogListUserId1 = "DialogList_" + userId1 // задать имя DialogList
    let Data1 = JSON.parse(localStorage.getItem(dialogListUserId1)); // запросить диалоглист с сервера по заданному имени
    if (!Data1) {
      Data1 = [] // если такого диалога на сервере нет, занулить его
    }
    const newDialogLocal = { // новый диалог в диалогЛисте
    ...
    if (shouldDataUpdate) { // если флаг обновления диалоглиста true, то добавляем диалог в диалогЛист
      const Data2 = [...Data1, newDialogLocal];
  ...
  getDialogListMyID: (myId) => { // получить мой диалогЛист с сервера, сформированный другими пользователями
    const dialogListUserId1 = "DialogList_" + myId // задать имя моего DialogList
  ...
  postDialog: (formDataNewMessage, myId, MyName, MyPhoto, userId) => { //отправка сообщения в LocalStorage
    ...
    const Dialog_2 = [...Dialog_1, {// добавить новое сообщение в запрошенный диалог
  ...
  deleteDialog: (dialogId, userId1, userId2) => { // удаления диалога из диалоглиста по его ID
    const Data1 = apiDialogs.getDialogListMyID(userId1)
  // console.log(Data1)
    const Data2 = Data1.filter(d=>d.dialogId!==dialogId)
    //console.log(Data2)
...
export const apiCommon = { // объект с методами api для общих нужд
  putTheme1: (theme1) => { // задание theme1 в localStorage
    ...
    const Data1 = apiCommon.getTheme1(); // запросить тему с localStorage после записи
