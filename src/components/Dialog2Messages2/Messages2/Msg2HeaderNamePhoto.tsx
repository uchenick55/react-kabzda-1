...
const Msg2HeaderNamePhoto:React.FC<Msg2HeaderNamePhotoType> = ({Dialog2All, userId}) => {
    const D2Item:newMessagesItem = Dialog2All.filter(d2=>d2.id===userId)[0]
    ...
    return <div>
        {D2Item && <div>
            ...
            <div className={classes.Msg2HeaderActivityDate} >Был(а): {" "}
                {D2Item && D2Item.lastUserActivityDate.slice(0, 10)}
            </div>
        </div>}
    </div>
}
